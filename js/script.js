const translations = {
  pt: {
    skip_link: "Pular para o conteúdo",
    nav_about: "Sobre",
    nav_experience: "Experiência",
    nav_work: "Projetos",
    nav_contact: "Contato",
    hero_label: "I / INTRODUÇÃO",
    hero_greeting: "Olá, meu nome é",
    hero_role: "FULL STACK DEVELOPER",
    hero_description:
      "Desenvolvo produtos digitais do front-end à infraestrutura, transformando problemas reais em software claro e sustentável.",
    hero_projects: "Ver projetos ↘",
    hero_cv: "Download CV ↗",
    hero_location: "Rio de Janeiro · Brasil",
    portrait_caption: "Construindo entre interface, sistema e nuvem.",
    about_label: "II / SOBRE",
    about_title:
      "Gosto de entender como as coisas funcionam — e depois descobrir como fazê-las funcionar melhor.",
    about_description:
      "Desenvolvedor Full Stack baseado no Rio de Janeiro. Trabalho entre interface, backend, arquitetura e cloud, buscando construir software que seja tão fácil de manter quanto de usar.",
    about_focus_label: "FOCUS",
    about_focus_value: "Full Stack Development",
    about_location_label: "LOCATION",
    about_location_value: "Rio de Janeiro, Brasil",
    about_languages_label: "LANGUAGES",
    about_languages_value: "Português / Inglês C1",
    about_current_label: "CURRENTLY",
    about_current_value: "Evoluindo o Finova",
    experience_label: "III / EXPERIÊNCIA",
    experience_title: "Experiência que sai da tela.",
    experience_doss_period: "2025 — PRESENTE",
    experience_doss_role: "FULL STACK DEVELOPER",
    experience_doss_intro:
      "Responsável pela evolução da plataforma imobiliária, da interface ao sistema em produção.",
    experience_doss_point_1:
      "Interface, UX, UI e SEO para uma jornada de busca mais clara.",
    experience_doss_point_2:
      "Autenticação, segurança de dados, favoritos e preferências persistentes.",
    experience_doss_point_3:
      "Painel administrativo com gestão de imóveis, leads, FAQ e notícias.",
    experience_impact: "leads semanais<br />de 50 para 100",
    experience_conversion: "conversão em<br />vendas mensais",
    independent_label: "INDEPENDENT WORK",
    independent_period: "2025 — PRESENTE",
    independent_description:
      "Projetos para negócios reais, da identidade e estratégia à experiência entregue em produção.",
    independent_action: "Conhecer trabalhos ↘",
    quote_text:
      "“A liberdade é pouco. O que eu desejo ainda não tem nome.”",
    quote_author: "Clarice Lispector",
    work_label: "IV / SELECTED WORK",
    work_title: "Projetos<br /><em>selecionados.</em>",
    work_description:
      "Alguns produtos que representam melhor como penso, construo e resolvo problemas.",
    finova_kicker: "PERSONAL FINANCE / SAAS-LIKE",
    finova_hook:
      "Uma aplicação financeira pessoal construída para transformar rotina, metas e gastos em uma visão mais clara.",
    finova_role: "Full Stack / Product",
    finova_idea:
      "Dar contexto aos dados financeiros antes de pedir uma decisão.",
    project_live: "Ver projeto ↗",
    project_code: "Código ↗",
    rego_kicker: "INDEPENDENT WORK / 2025",
    rego_hook:
      "Identidade e experiência digital para uma consultoria de cidadania europeia e serviços internacionais.",
    rego_role: "Design / Front-End",
    rego_idea:
      "Organizar informação complexa em uma jornada direta para contato.",
    kidilicia_kicker: "EM DESENVOLVIMENTO",
    kidilicia_hook:
      "Um cardápio digital pensado para encurtar o caminho entre fome, escolha e pedido.",
    kidilicia_role: "Front-End / Product",
    kidilicia_idea:
      "Guiar escolhas e entregar o pedido completo pelo WhatsApp.",
    tools_label: "V / TOOLS",
    tools_title: "Ferramentas mudam.<br /><em>Fundamentos permanecem.</em>",
    tools_quality: "QUALIDADE",
    tools_process: "PROCESSO",
    learning_label: "VI / LEARNING",
    learning_title: "Aprender também<br /><em>faz parte do trabalho.</em>",
    education_title: "Análise e Desenvolvimento de Sistemas",
    education_detail: "5º período · Formação em andamento",
    cert_1_title: "Curso Básico Frontend",
    cert_1_detail: "Rafael Ferreira / Hotmart · 02/2026",
    cert_2_title: "Curso Backend",
    cert_2_detail: "Rafael Ferreira / Hotmart · 03/2026",
    cert_3_title: "Desenvolvimento Web com Python e Django",
    cert_3_detail: "Jamilton Damasceno / Udemy · 05/2026",
    cert_4_title: "Curso Banco de Dados",
    cert_4_detail: "Rafael Ferreira / Hotmart · 05/2026",
    contact_label: "VII / CONTATO",
    contact_title: "Vamos construir<br /><em>alguma coisa?</em>",
    contact_description:
      "Aberto a oportunidades, projetos e boas conversas sobre tecnologia.",
    contact_cv_label: "CURRÍCULO",
    contact_cv_value: "Baixar PDF",
    footer_text: "Feito com intenção, HTML, CSS e JavaScript.",
  },
  en: {
    skip_link: "Skip to content",
    nav_about: "About",
    nav_experience: "Experience",
    nav_work: "Work",
    nav_contact: "Contact",
    hero_label: "I / INTRODUCTION",
    hero_greeting: "Hello, my name is",
    hero_role: "FULL STACK DEVELOPER",
    hero_description:
      "I build digital products from front-end to infrastructure, turning real problems into clear, sustainable software.",
    hero_projects: "View projects ↘",
    hero_cv: "Download CV ↗",
    hero_location: "Rio de Janeiro · Brazil",
    portrait_caption: "Building across interface, system and cloud.",
    about_label: "II / ABOUT",
    about_title:
      "I like understanding how things work — then figuring out how to make them work better.",
    about_description:
      "Full Stack Developer based in Rio de Janeiro. I work across interface, backend, architecture and cloud, building software that is as easy to maintain as it is to use.",
    about_focus_label: "FOCUS",
    about_focus_value: "Full Stack Development",
    about_location_label: "LOCATION",
    about_location_value: "Rio de Janeiro, Brazil",
    about_languages_label: "LANGUAGES",
    about_languages_value: "Portuguese / English C1",
    about_current_label: "CURRENTLY",
    about_current_value: "Evolving Finova",
    experience_label: "III / EXPERIENCE",
    experience_title: "Experience beyond the screen.",
    experience_doss_period: "2025 — PRESENT",
    experience_doss_role: "FULL STACK DEVELOPER",
    experience_doss_intro:
      "Responsible for evolving the real estate platform, from interface to production system.",
    experience_doss_point_1:
      "Interface, UX, UI and SEO for a clearer search journey.",
    experience_doss_point_2:
      "Authentication, data security, favorites and persistent preferences.",
    experience_doss_point_3:
      "Admin panel for properties, leads, FAQ and news management.",
    experience_impact: "weekly leads<br />from 50 to 100",
    experience_conversion: "conversion in<br />monthly sales",
    independent_label: "INDEPENDENT WORK",
    independent_period: "2025 — PRESENT",
    independent_description:
      "Projects for real businesses, from identity and strategy to a production-ready experience.",
    independent_action: "View work ↘",
    quote_text:
      "“Freedom is little. What I desire still has no name.”",
    quote_author: "Clarice Lispector",
    work_label: "IV / SELECTED WORK",
    work_title: "Selected<br /><em>work.</em>",
    work_description:
      "A few products that best represent how I think, build and solve problems.",
    finova_kicker: "PERSONAL FINANCE / SAAS-LIKE",
    finova_hook:
      "A personal finance app built to turn routines, goals and expenses into a clearer picture.",
    finova_role: "Full Stack / Product",
    finova_idea:
      "Give financial data context before asking for a decision.",
    project_live: "View project ↗",
    project_code: "Code ↗",
    rego_kicker: "INDEPENDENT WORK / 2025",
    rego_hook:
      "Digital identity and experience for a European citizenship and international services consultancy.",
    rego_role: "Design / Front-End",
    rego_idea:
      "Organize complex information into a direct journey to contact.",
    kidilicia_kicker: "IN DEVELOPMENT",
    kidilicia_hook:
      "A digital menu designed to shorten the journey between hunger, choice and order.",
    kidilicia_role: "Front-End / Product",
    kidilicia_idea:
      "Guide choices and send the complete order through WhatsApp.",
    tools_label: "V / TOOLS",
    tools_title: "Tools change.<br /><em>Fundamentals remain.</em>",
    tools_quality: "QUALITY",
    tools_process: "PROCESS",
    learning_label: "VI / LEARNING",
    learning_title: "Learning is also<br /><em>part of the work.</em>",
    education_title: "Systems Analysis and Development",
    education_detail: "5th semester · Degree in progress",
    cert_1_title: "Basic Frontend Course",
    cert_1_detail: "Rafael Ferreira / Hotmart · 02/2026",
    cert_2_title: "Backend Course",
    cert_2_detail: "Rafael Ferreira / Hotmart · 03/2026",
    cert_3_title: "Web Development with Python and Django",
    cert_3_detail: "Jamilton Damasceno / Udemy · 05/2026",
    cert_4_title: "Database Course",
    cert_4_detail: "Rafael Ferreira / Hotmart · 05/2026",
    contact_label: "VII / CONTACT",
    contact_title: "Shall we build<br /><em>something?</em>",
    contact_description:
      "Open to opportunities, projects and good conversations about technology.",
    contact_cv_label: "RESUME",
    contact_cv_value: "Download PDF",
    footer_text: "Built with intention, HTML, CSS and JavaScript.",
  },
};

const root = document.documentElement;
const languageToggle = document.getElementById("lang-toggle");
const themeToggle = document.getElementById("theme-toggle");
const menuToggle = document.getElementById("menu-toggle");
const navPanel = document.getElementById("site-nav-links");
const navLinks = [...document.querySelectorAll(".nav-panel a")];

function applyLanguage(language) {
  const dictionary = translations[language];

  document.querySelectorAll("[data-i18n]").forEach((element) => {
    const value = dictionary[element.dataset.i18n];
    if (value) element.textContent = value;
  });

  document.querySelectorAll("[data-i18n-html]").forEach((element) => {
    const value = dictionary[element.dataset.i18nHtml];
    if (value) element.innerHTML = value;
  });

  root.lang = language === "pt" ? "pt-BR" : "en";
  document.querySelector(".lang-current").textContent = language.toUpperCase();
  localStorage.setItem("language", language);
}

function applyTheme(theme) {
  root.dataset.theme = theme;
  localStorage.setItem("theme", theme);
}

const savedLanguage = localStorage.getItem("language") || "pt";
const savedTheme = localStorage.getItem("theme") || "dark";
applyLanguage(savedLanguage);
applyTheme(savedTheme);

languageToggle.addEventListener("click", () => {
  applyLanguage((localStorage.getItem("language") || "pt") === "pt" ? "en" : "pt");
});

themeToggle.addEventListener("click", () => {
  applyTheme((root.dataset.theme || "dark") === "dark" ? "light" : "dark");
});

function setMenu(open) {
  navPanel.classList.toggle("is-open", open);
  menuToggle.classList.toggle("is-open", open);
  menuToggle.setAttribute("aria-expanded", String(open));
}

menuToggle.addEventListener("click", () => setMenu(!navPanel.classList.contains("is-open")));
navLinks.forEach((link) => link.addEventListener("click", () => setMenu(false)));

document.addEventListener("keydown", (event) => {
  if (event.key === "Escape") setMenu(false);
});

const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
const revealElements = document.querySelectorAll(".reveal");

if (reducedMotion) {
  revealElements.forEach((element) => element.classList.add("is-visible"));
} else {
  const revealObserver = new IntersectionObserver(
    (entries, observer) => {
      entries.forEach((entry) => {
        if (!entry.isIntersecting) return;
        entry.target.classList.add("is-visible");
        observer.unobserve(entry.target);
      });
    },
    { threshold: 0.12 },
  );

  revealElements.forEach((element) => revealObserver.observe(element));
}

const chapters = [...document.querySelectorAll("main section.chapter[id]")];
const activeObserver = new IntersectionObserver(
  (entries) => {
    const visibleEntry = entries.find((entry) => entry.isIntersecting);
    if (!visibleEntry) return;

    navLinks.forEach((link) => {
      link.classList.toggle("active", link.getAttribute("href") === `#${visibleEntry.target.id}`);
    });
  },
  { rootMargin: "-40% 0px -55% 0px", threshold: 0 },
);

chapters.forEach((chapter) => activeObserver.observe(chapter));
